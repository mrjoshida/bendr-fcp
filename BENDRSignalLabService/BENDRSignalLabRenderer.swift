// BENDRSignalLabRenderer.swift — Metal Render Pipeline for BENDR Signal Lab

import Foundation
import Metal
import FxPlug

enum BENDRSignalLabRenderer {

    static func render(destination: FxImageTile, source: FxImageTile, params: SignalLabParams) throws {
        ...
        guard let library = try? context.device.makeDefaultLibrary(bundle: Bundle(for: BENDRSignalLabFilter.self)) ?? context.device.makeDefaultLibrary() else {
            throw BendrError.shaderNotFound("Default library for BENDRSignalLab")
        }

        let pipeline = try context.computePipeline(named: "bendrSignalLab", from: library)
        ...
        encoder.setBytes(&mutableParams, length: MemoryLayout<SignalLabParams>.stride, index: 0)
