// BENDRSynthRenderer.swift — Metal Render Pipeline for BENDR Synth

import Foundation
import Metal
import FxPlug

enum BENDRSynthRenderer {

    static func render(destination: FxImageTile, source: FxImageTile, params: SynthParams) throws {
        ...
        guard let library = try? context.device.makeDefaultLibrary(bundle: Bundle(for: BENDRSynthFilter.self)) ?? context.device.makeDefaultLibrary() else {
            throw BendrError.shaderNotFound("Default library for BENDRSynth")
        }

        let pipeline = try context.computePipeline(named: "bendrSynth", from: library)
        ...
        encoder.setBytes(&mutableParams, length: MemoryLayout<SynthParams>.stride, index: 0)
